...
### feature extraction ###
df1 = pd.read_csv('/Users/Jia/Documents/nanana/course/CSE 347/project/datasets/train_label.csv', header=0)
...
for k in xrange(num):
    if k == 0:
        csv = [userID[k], merchantID[k], feature_age[k], feature_gender[k], feature_score_total[k], feature_score_mc[k], feature_ratio_bf11[k], feature_ratio_on11[k], feature_ratiodf[k], feature_ratio_mc[k], prob[k]]
    else:
        newrow = [userID[k], merchantID[k], feature_age[k], feature_gender[k], feature_score_total[k], feature_score_mc[k], feature_ratio_bf11[k], feature_ratio_on11[k], feature_ratiodf[k], feature_ratio_mc[k], prob[k]]
        csv = np.vstack([csv, newrow])   
'''
X_train = {"age": feature_age, "gender": feature_gender, "score_total": feature_score_total, "score_merchant": feature_score_mc, "ratio_before11": feature_ratio_bf11, "ratio_on11": feature_ratio_on11, "ratio_difference": feature_ratiodf, "ratio_merchant": feature_ratio_mc}
df4 = pd.DataFrame(X_train, columns=["age", "gender", "score_total", "score_merchant","ratio_before11", "ratio_on11", "ratio_difference", "ratio_merchant"])
df4.to_csv("X_train.csv", index=False)

y_train = {"prob": df1["prob"].values}
df5 = pd.DataFrame(y_train, columns=["prob"])
df5.to_csv("y_train.csv", index=False)
